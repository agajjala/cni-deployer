...
resource aws_iam_role_policy_attachment private_link_event_handler_xray_access {
  role       = aws_iam_role.private_link_event_handler.name
  policy_arn = local.xray_write_access_arn
}

resource aws_iam_role_policy_attachment private_link_event_handler_dynamodb_read_write_access {
  role       = aws_iam_role.private_link_event_handler.name
  policy_arn = aws_iam_policy.inbound_dynamodb_read_write_access.arn
}

resource aws_iam_role_policy_attachment private_link_event_handler_private_link_limited_access {
  role       = aws_iam_role.private_link_event_handler.name
