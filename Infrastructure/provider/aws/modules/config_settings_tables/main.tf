...
resource aws_dynamodb_table_item outbound_infra_vpcs {
  ...
  item = jsonencode({
    ...
    "Payload" = {
      "S" : "[{\"vpc_id\": \"${var.outbound_vpc.id}\", \"subnet_ids\": ${jsonencode(var.outbound_private_subnets[*].id)}, \"proxy_url\": \"${format("https://%s:443", var.outbound_nlb.dns_name)}\", \"status\": \"inService\", \"total_capacity\": 490, \"security_group_ids\": [\"${var.nginx_sg.id}\"]}]"
    }
  })
}
