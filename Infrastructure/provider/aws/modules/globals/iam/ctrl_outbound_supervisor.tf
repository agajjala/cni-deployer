resource aws_iam_role outbound_supervisor {
  name = "${var.resource_prefix}-out-super"
...
resource aws_iam_role_policy_attachment outbound_supervisor_private_link_access {
  role       = aws_iam_role.outbound_supervisor.name
  policy_arn = aws_iam_policy.private_link_access.arn
}

resource aws_iam_role_policy_attachment outbound_supervisor_route53_read_write_access {
  role       = aws_iam_role.outbound_supervisor.name
